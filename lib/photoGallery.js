...
const { withHandler } = require("./handler");
...
// Creates a photo-CRUD API handler backed by its own store key, so
// independent galleries (e.g. the main gallery and the events gallery)
// can share identical upload/category/price/delete behavior.
function createPhotoGalleryHandler(storeKey) {
  return withHandler(async (req, res) => {
    res.setHeader("Content-Type", "application/json");

    if (req.method === "GET") {
      const photos = await store.get(storeKey, []);
    ...
    if (req.method === "POST") {
      ...
      const photos = await store.get(storeKey, []);
      photos.push(photo);
      await store.set(storeKey, photos);
    ...
    if (req.method === "DELETE") {
      ...
      await store.set(storeKey, remaining);
...
module.exports = { createPhotoGalleryHandler };
